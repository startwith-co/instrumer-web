...
import { useRegisterConsumerMutation, useSendEmailMutation, useVerifyEmailAuthKeyMutation } from '@/lib/b2b-auth';
import { useRouter } from 'next/navigation';
import { useState } from 'react';
import { useForm } from 'react-hook-form';

interface IConsumerFormData {
  businessName: string;
  managerName: string;
  phone: string;
  email: string;
...
const ConsumerRegisterForm = () => {
  ...
  const { mutateAsync: registerConsumer, isPending: isRegistering } = useRegisterConsumerMutation();

  const form = useForm<IConsumerFormData>({
    defaultValues: {
      businessName: '',
      managerName: '',
      phone: '',
  ...
  // 회원가입 제출
  const onSubmit = async (data: IConsumerFormData) => {
    if (!isEmailVerified) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
      ...
      {/* 이메일 */}
      <div>
        <label className="mb-2 block text-sm text-gray-600">사용자 이메일</label>
